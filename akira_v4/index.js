...
const { startupPassword } = require('./empirestore/token');

const AUTH_FILE = './auth.json';
const PAIRING_DIR = './empirestore/pairing/';
...
const initializeBot = async () => {
    ...
    console.log(chalk.green('Akira md v1'));
...
function launchBot() {
    console.clear();
    console.log(chalk.green('empire solos....\n'));
    ...
    if (fs.existsSync(botPath)) {
        try {
            ...
            require('./bot');
            telegramLoaded = true;
            console.log(chalk.green('✅ Akira md ᴀᴄᴛɪᴠᴇ'));
        } catch (error) {
    ...
    console.log(telegramLoaded ? chalk.green( 'Akira md : ᴀᴄᴛɪᴠᴇ ✅') : chalk.red('❌ Akira x 2025'));
    console.log(whatsappLoaded ? chalk.green('✅ ᴡʜᴀᴛsᴀᴘᴘ ᴄᴏᴍᴍᴀɴᴅs: ᴀᴄᴛɪᴠᴇ') : chalk.red('❌ ᴡʜᴀᴛsᴀᴘᴘ ᴄᴏᴍᴍᴀᴍᴅs : ɪɴᴀᴄtɪᴠᴇ'));
    console.log(chalk.cyan('⚄︎════════════════════════════════⚄︎\n'));
