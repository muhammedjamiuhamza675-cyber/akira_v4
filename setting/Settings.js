...
function getSetting(chat, key, defaultValue = false) {
  if (!global.settings) global.settings = {}
  if (!global.settings[chat]) global.settings[chat] = {}
  return global.settings[chat][key] !== undefined ? global.settings[chat][key] : defaultValue
}

function setSetting(chat, key, value) {
  if (!global.settings) global.settings = {}
  if (!global.settings[chat]) global.settings[chat] = {}
  global.settings[chat][key] = value
  
  // Save to file
  fs.writeFileSync('./settings.json', JSON.stringify(global.settings, null, 2))
}
